...
import { Edit, Trash2, Plus, Package, AlertTriangle, X } from "lucide-react";
...
const PlatformPanel: React.FC = () => {
  ...
  const handlePurchaseSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Simply update platform inventory (simplified approach)
      const { error: updateError } = await supabase
        .from("game_coins")
        .update({
          inventory: selectedPlatform.inventory + purchaseForm.quantity,
        })
        .eq("id", selectedPlatform.id);
